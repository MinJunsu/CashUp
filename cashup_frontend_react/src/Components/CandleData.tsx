...
const HeaderElement = styled.div<{ flag: boolean }>`
	...
	background-color: ${(props) =>
		props.flag ? "rgba(209, 207, 207, 0.6)" : "transparent"};
	:hover {
	...
	return (
		<Container>
			<Header>
				<HeaderElement
					id='minute'
					onClick={(event) => setIsMinute(true)}
					flag={isMinute}
				>
					<h4>5분 데이터</h4>
				</HeaderElement>
				<HeaderElement
					id='hour'
					onClick={(event) => setIsMinute(false)}
					flag={!isMinute}
				>
					<h4>1시간 데이터</h4>
				</HeaderElement>
			...
			{elementData.loading ? (
				<LoaderCotainer>
					<Loader type='TailSpin' color='#a9eef5' height={100} width={100} />
				</LoaderCotainer>
			) : (
				<Frame>
					<Section>
						<SectionHeader>
							...
							<SectionHeaderElement basis={1}>4시간 UD</SectionHeaderElement>
							...
							{elementData.data?.map((element) => {
								return (
									<SectionBodyContainer
										key={element.datetime.replace("T", " ").substring(0, 16)}
										id={element.datetime.replace("T", " ").substring(0, 16)}
									>
										<SectionBodyElement basis={4}>
											{element.datetime.replace("T", " ").substring(0, 16)}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.open_price}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.max_price}
										...
										<SectionBodyElement basis={1}>
											{element.continue_up_down}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.hour_up_down}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.signal}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.work_one_1_0}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.work_one_1_5}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.work_two_1_0}
										</SectionBodyElement>
										<SectionBodyElement basis={1}>
											{element.work_two_1_5}
										</SectionBodyElement>
									</SectionBodyContainer>
								);
							})}
